...
class BHTTUCKER3_test(object):
    def __init__(self,X1, X2, Us,Rs, K, tol, seed=None, Us_mode=4, \
...
#        begin_idx = self._p + self._q
    ...
    def run(self):
        ...
        Us,core,corest,loss    = self._run()
          
        if self._convergence_loss:
            
            return Us,core,corest,loss      
        
        return  Us,core,corest,None
...
#得到更新后的源域和目标域的核张量             
        ...
        # X_S = tl.tenalg.multi_mode_dot(coress, Us)
        # X_T = tl.tenalg.multi_mode_dot(corest, Us)

        # X_S = torch.stack(coress).permute(1,2,0)
        # X_T = torch.stack(corest).permute(1,2,0)
...
#        X_S7 = self._inverse_MDT(mdt7, new_X_source7, self._taus, ori_shape7)
        ...
        return Us,core,corest,con_loss 