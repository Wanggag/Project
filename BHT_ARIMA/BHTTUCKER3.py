...
#        begin_idx = self._p + self._q
    ...
    def run(self):
        ...
        Us,coress, corest,X_S,X_T,loss    = self._run()
          
        if self._convergence_loss:
            
            return Us,coress, corest,X_S,X_T,loss      
        
        return  Us,coress, corest,X_S,X_T,None
...
#得到更新后的源域和目标域的核张量             
        ...
        coress1 = torch.cat([torch.unsqueeze(i,0) for i in coress], dim=0)
        corest1 = torch.cat([torch.unsqueeze(i,0) for i in corest], dim=0)
        
        coress1 = coress1.permute(1,2,0)
        corest1 = corest1.permute(1,2,0)
        
        X_S = tl.tenalg.multi_mode_dot(coress1, Us)
...
#        X_S7 = self._inverse_MDT(mdt7, new_X_source7, self._taus, ori_shape7)
        ...
        return Us,coress, corest,X_S,X_T,con_loss 